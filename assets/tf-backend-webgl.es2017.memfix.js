...
(function (global, factory) {
   ...
   * =============================================================================
   */
  const { addImpl: addImplCPU, ceilImpl: ceilImplCPU, expImpl: expImplCPU, expm1Impl: expm1ImplCPU, floorImpl: floorImplCPU, greaterImpl: greaterImplCPU, lessImpl: lessImplCPU, logImpl: logImplCPU, maxImpl: maxImplCPU, multiplyImpl: multiplyImplCPU, negImpl: negImplCPU, prodImpl: prodImplCPU, rsqrtImpl: rsqrtImplCPU, simpleAbsImpl: simpleAbsImplCPU, sliceImpl: sliceImplCPU, stridedSliceImpl: stridedSliceImplCPU, subImpl: subImplCPU, transposeImpl: transposeImplCPU, uniqueImpl: uniqueImplCPU, } = shared;
...
`;
  ...
  class MathBackendWebGL extends tf.KernelBackend {
      ...
      neg(x) {
          if (this.shouldExecuteOnCPU([x])) {
              const [outVals, newShape] = negImplCPU(this.texData.get(x.dataId).values, x.shape, x.dtype);
      ...
      prod(x, axes) {
          if (this.shouldExecuteOnCPU([x])) {
              const xVals = this.texData.get(x.dataId).values;
              const { outVals, outShape, outDtype } = prodImplCPU(x.shape, x.dtype, xVals, axes);
      ...
      less(a, b) {
          if (this.shouldExecuteOnCPU([a, b])) {
              const aVals = this.texData.get(a.dataId).values;
              const bVals = this.texData.get(b.dataId).values;
              const [outVals, newShape] = lessImplCPU(a.shape, b.shape, aVals, bVals, 'bool');
      ...
      greater(a, b) {
          if (this.shouldExecuteOnCPU([a, b])) {
              const aVals = this.texData.get(a.dataId).values;
              const bVals = this.texData.get(b.dataId).values;
              const [outVals, newShape] = greaterImplCPU(a.shape, b.shape, aVals, bVals, 'bool');
              return this.makeOutput(newShape, 'bool', outVals);
          }
          if (tf.env().getBool('WEBGL_PACK_BINARY_OPERATIONS')) {
      ...
      softmax(logits, dim) {
          ...
          const b = this.exp(a);
          const sumExp = tf.sum(b, axes).reshape(expandedShape);
      ...
      resizeBilinear(x, newHeight, newWidth, alignCorners, halfPixelCenters) {
          const program = tf.env().getBool('WEBGL_PACK_IMAGE_OPERATIONS') ?
              new ResizeBilinearPackedProgram(x.shape, newHeight, newWidth, alignCorners, halfPixelCenters) :
              new ResizeBilinearProgram(x.shape, newHeight, newWidth, alignCorners, halfPixelCenters);
          return this.compileAndRun(program, [x], 'float32');
      }
...
`;
  ...
  function mapActivationToShaderProgram$1(activation, packed = false) {
      if (activation === 'linear') {
          if (packed) {
              return LINEAR$1;
          }
  ...
  function batchMatMulImpl({ a, b, transposeA, transposeB, backend, bias = null, preluActivationWeights = null, activation = null }) {
      ...
      const fusedActivation = activation != null ?
          mapActivationToShaderProgram$1(activation, true) :
          null;
      const containsFusedOps = hasBias || hasPreluActivationWeights || fusedActivation != null;
      let out;
      ...
      if ((outerShapeA === 1 || outerShapeB === 1) &&
          ...
          const dtype = tf.upcastType(a.dtype, b.dtype);
          const program = new MatMulPackedProgram(a3dShape, b3dShape, [batchDim, outerShapeA, outerShapeB], transposeA, transposeB, hasBias, fusedActivation, hasPreluActivationWeights);
...
`;
  ...
  // remapping).
  function conv2dByMatMul({ x, filter, convInfo, backend, bias = null, preluActivationWeights = null, activation = null }) {
  ...
  function conv2dWithIm2Row({ x, filter, convInfo, backend, bias = null, preluActivationWeights = null, activation = null }) {
      ...
      const hasBias = bias != null;
      const hasPreluActivationWeights = preluActivationWeights != null;
      const fusedActivation = activation ? mapActivationToShaderProgram$1(activation, true) : null;
      const matmulProgram = new MatMulPackedProgram(im2ColReshaped.shape, w2Row.shape, [1, numCols, convInfo.outChannels], transposeA, transposeB, hasBias, fusedActivation, hasPreluActivationWeights);
...
`;
  ...
  function fusedConv2d(args) {
      ...
      if (convInfo.filterHeight === 1 && convInfo.filterWidth === 1 &&
          ...
          const hasBias = bias != null;
          const hasPreluActivationWeights = preluActivationWeights != null;
          const fusedActivation = activation ? mapActivationToShaderProgram$1(activation, false) : null;
...
`;
  ...
  const square = unaryKernelFunc(SQUARE);
  const squareConfig = {
  ...
  const subKernelFunc = binaryKernelFunc({
      opSnippet: SUB,
      packedOpSnippet: SUB,
      supportsComplex: true,
   ...
   * =============================================================================
   */
  const tile = ({ inputs, backend, attrs }) => {
      const { x } = inputs;
      const { reps } = attrs;
      if (x.dtype === 'string') {
          ...
          return tf.kernel_impls.tile(buf, reps);
      }
