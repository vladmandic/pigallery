...
async function detectOld(model, image) {
  ...
  function buildDetectedObjects(labels, batched, result, maxScores, classes, index) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      objects.push({ box, class: labels[id].displayName, score: maxScores[indexes[i]], id });
    }
    return objects;
  }
  ...
  const results = buildDetectedObjects(model.labels, batched, result, scores, classes, index); // disposes of batched, result, index
  tf.dispose(imgBuf);
  tf.dispose(reshaped);
  return results;
}
...
async function exec(model, image) {
  let result;
  if (model.config.exec === 'old') result = await detectOld(model, image);
  else result = await detect(model, image);
  return result;
}
